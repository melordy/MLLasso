...
def ROCCurve(Xinput,Yinput,graphType):
	...
	for train, test in cv.split(Xinput, Yinput):
		...
		#print("YSCORE")
		flat_list = [item for sublist in y_score for item in sublist] #make into flat list
		#print(flat_list[:10])
		#print(len(flat_list[:10]))        
		...
		roc_auc = auc(fpr,tpr)
		aucs.append(roc_auc)
		plt.plot(fpr, tpr, lw=1, label='ROC curve for fold {0}, area:{1:0.2f}'''.format((i+1),roc_auc))
